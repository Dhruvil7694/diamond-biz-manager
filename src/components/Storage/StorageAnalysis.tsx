    ...
    try {
      ...
      for (const bucket of buckets) {
        ...
        for (const file of files || []) {
          ...
          // Since we don't have direct access to file metadata with size,
          // we'll estimate file size based on name length or use a default value
          // In a real implementation, you would need a more accurate way to get file sizes
          const fileExtension = getFileExtension(file.name);
          const estimatedSize = file.name.length * 1024; // Very rough estimation
          const lastAccessed = file.updated_at || file.created_at || new Date().toISOString();
          ...
          fileTypeDistribution[fileExtension].size += estimatedSize;

          // Check for large files (estimated > 10MB for demo purposes)
          ...
          const simpleHash = `${fileExtension}-${file.name.length}`;
